...
(define (get-crime-by-state department [test #t]) 
    ...
    (if (or test-mode test)
        (let* [(result (query-list database 
                                  (format "SELECT data FROM crime WHERE state=? LIMIT ~a" hardware-limit)
                                department))
               (result (map string->jsexpr result))]
           (map json->report result)
           )(let* [(result (query-list database 
                                   "SELECT data FROM crime WHERE state=?"
                                department
                                ))
               (result (map string->jsexpr result))
               ]
...
(define (main)
    ...
    (displayln "Production get-crime-by-state")
    (displayln (length (time (get-crime-by-state "Alabama" false))))
    (displayln "Test get-crime-by-state")
    (displayln (length (time (get-crime-by-state "Alabama" true))))
