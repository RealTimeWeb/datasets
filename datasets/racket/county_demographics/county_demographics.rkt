...
(define (get-counties-by-state state [test #t]) 
    (check-arg 'state (string? state) 'string 1 state)
     

    (if (or test-mode test)
        (let* [(result (query-list database 
                                  (format "SELECT data FROM demographics WHERE state=? COLLATE NOCASE LIMIT ~a" hardware-limit)
                                state))
               (result (map string->jsexpr result))]
           (map json->report result)
           )(let* [(result (query-list database 
                                   "SELECT data FROM demographics WHERE state=? COLLATE NOCASE"
                                state
                                ))
               (result (map string->jsexpr result))
               ]
...
(define (main)
    ...
    (displayln "Production get-counties-by-state")
    (displayln (length (time (get-counties-by-state "'VA'" false))))
    (displayln "Test get-counties-by-state")
    (displayln (length (time (get-counties-by-state "'VA'" true))))
