...
_Constants._DATABASE_NAME = "suicideattacks.db"
...
def get_attacks(test=True):
    """
    Returns a list of the attacks in the database.
    
    """
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM suicideattacks LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM suicideattacks".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
