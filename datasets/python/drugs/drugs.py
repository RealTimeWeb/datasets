...
def get_surveys(question):
    ...
    potentials = [r[0].lower() for r in _DATABASE.execute("SELECT DISTINCT name FROM drugs").fetchall()]
    ...
    if False:
        ...
        rows = _DATABASE.execute("SELECT data FROM drugs WHERE name=? COLLATE NOCASE".format(
            hardware=_HARDWARE),
            (question, ))
