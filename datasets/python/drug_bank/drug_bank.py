...
class _Constants(object):
    ...
    '''
    _HEADER = {'User-Agent': 
              'CORGIS Drug Bank library for educational purposes'}
...
_Constants._DATABASE_NAME = "drug_bank.db"
...
def get_drugs(test=False):
    """
    Returns data about all the drugs in the database.
    
    """
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM drug_bank LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM drug_bank".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
...
def _test_interfaces():
    ...
    # Test test
    print("Test get_drugs")
    start_time = _default_timer()
    result = get_drugs(test=True)
