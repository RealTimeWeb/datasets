...
_Constants._DATABASE_NAME = "scripts.db"
...
def get_script_by_title(title):
    ...
    potentials = [r[0].lower() for r in _Constants._DATABASE.execute("SELECT DISTINCT title FROM scripts").fetchall()]
    ...
    if False:
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM scripts WHERE title=? COLLATE NOCASE LIMIT 1".format(
            hardware=_Constants._HARDWARE),
            (title, ))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r))['data'] for r in data]
