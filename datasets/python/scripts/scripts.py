...
def get_scripts(test=True):
    ...
    if _TEST or test:
        rows = _DATABASE.execute("SELECT data FROM scripts LIMIT {hardware}".format(
            hardware=_HARDWARE))
        ...
        rows = _DATABASE.execute("SELECT data FROM scripts".format(
            hardware=_HARDWARE))
        data = [r[0] for r in rows]
        data = [_json.loads(r)['data'] for r in data]
...
def get_script_by_title(title):
    ...
    potentials = [r[0].lower() for r in _DATABASE.execute("SELECT DISTINCT title FROM scripts").fetchall()]
    ...
    if False:
        ...
        rows = _DATABASE.execute("SELECT data FROM scripts WHERE title=? COLLATE NOCASE LIMIT 1".format(
            hardware=_HARDWARE),
            (title, ))
        data = [r[0] for r in rows]
        data = [_json.loads(r)['data'] for r in data]
