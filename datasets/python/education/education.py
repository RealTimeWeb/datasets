...
def get_state(state):
    ...
    potentials = [r[0].lower() for r in _DATABASE.execute("SELECT DISTINCT state FROM school").fetchall()]
    ...
    if False:
        ...
        rows = _DATABASE.execute("SELECT data FROM school WHERE state=? COLLATE NOCASE".format(
            hardware=_HARDWARE),
            (state, ))
...
def get_all_states():
    ...
    if False:
        ...
        rows = _DATABASE.execute("SELECT data FROM school".format(
            hardware=_HARDWARE))
        data = [r[0] for r in rows]
