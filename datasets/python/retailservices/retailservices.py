...
_Constants._DATABASE_NAME = "retailservices.db"
...
def get_report(test=True):
    """
    Returns sales, inventory, and ratio data for every month.
    
    """
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM retailservices LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM retailservices".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
