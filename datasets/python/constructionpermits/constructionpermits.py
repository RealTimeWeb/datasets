...
_Constants._DATABASE_NAME = "constructionpermits.db"
...
def get_records(test=True):
    ...
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM constructionpermits LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM constructionpermits".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
