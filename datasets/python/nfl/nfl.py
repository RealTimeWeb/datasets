...
_Constants._DATABASE_NAME = "nfl.db"
...
def get_games_by_home_team(home_team):
    ...
    potentials = [r[0].lower() for r in _Constants._DATABASE.execute("SELECT DISTINCT home_team FROM games").fetchall()]
    ...
    if False:
        ...
        rows = _Constants._DATABASE.execute("SELECT data from games WHERE home_team=? COLLATE NOCASE".format(
...
def get_games_by_away_team(away_team):
    ...
    # Match it against recommend values
    potentials = [r[0].lower() for r in _Constants._DATABASE.execute("SELECT DISTINCT away_team FROM games").fetchall()]
    ...
    if False:
        ...
        rows = _Constants._DATABASE.execute("SELECT data from games WHERE away_team=? COLLATE NOCASE".format(
            hardware=_Constants._HARDWARE),
            (away_team, ))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
...
def get_games(test=True):
    ...
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data from games LIMIT {hardware}".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM games".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
