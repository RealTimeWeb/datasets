...
_Constants._DATABASE_NAME = "soccer.db"
...
def get_games(league):
    ...
    potentials = [r[0].lower() for r in _Constants._DATABASE.execute("SELECT DISTINCT name FROM soccer").fetchall()]
    ...
    if False:
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM soccer WHERE name=?".format(
