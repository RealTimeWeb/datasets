...
def get_airports(test=True):
    ...
    if _TEST or test:
        rows = _DATABASE.execute("SELECT data FROM airports LIMIT {hardware}".format(
            hardware=_HARDWARE))
        ...
        rows = _DATABASE.execute("SELECT data FROM airports".format(
            hardware=_HARDWARE))
        data = [r[0] for r in rows]
        data = [_json.loads(r) for r in data]
