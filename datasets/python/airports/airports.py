...
class _Constants(object):
    _HEADER = {'User-Agent': 
              'CORGIS Airports library for educational purposes'}
...
_Constants._DATABASE_NAME = "airports.db"
...
def get_airports(test=True):
    """
    Returns a list of airports around the world.
    
    """
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM airports LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM airports".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
...
def _test_interfaces():
    ...
    # Production test
    print("Production get_airports")
    start_time = _default_timer()
    result = get_airports(test=False)
