...
class _Constants(object):
    _HEADER = {'User-Agent': 
              'CORGIS Drugbank library for educational purposes'}
...
_Constants._DATABASE_NAME = "drugbank.db"
...
def get_drugs(test=True):
    ...
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM drugbank LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM drugbank".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
